...
def find_activities(type_activities: list, df_dict: dict, df: pd.DataFrame, gmaps: googlemaps.client.Client) -> pd.DataFrame:
    ...
    for type_activity in type_activities:
        ...
        print(f"Found {len(nearby_search_results_list)}x {type_activity} in a radius of {activity_radius / 1000} km")
...
def process():
    ...
    for index, _ in raw_df.iterrows():
        ...
        output_df = find_activities(settings.type_activities, df_dict, output_df, gmaps)
        logger.info("=====================================")
